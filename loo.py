    ...
    for uploaded_file in uploaded_files:
        with uploaded_file as file:
            ...
            if 'LogType' in df.columns:
                if 10 not in df['LogType'].unique():
                    st.write(f"文件 {uploaded_file.name} 中无数据")
        ...
        for index, row in filtered_df.iterrows():
            ...
            if prev_log_type_was_10 and log_type != 10:
                ...
                prev_route_number = prev_row.iloc[42]
        ...
        # 计算纠偏量
        masked_df['Deviation'] = masked_df['MotorCommand_SteeringAngle1'] - masked_df['RefTheta']

        # 计算平均纠偏量
        average_deviation = masked_df['Deviation'].mean()

        # 计算最大纠偏量（保留正负号）
        max_deviation = masked_df['Deviation'].abs().idxmax()  # 找到绝对值最大的纠偏量的索引
        max_deviation_value = masked_df.at[max_deviation, 'Deviation']  # 获取该索引处的纠偏量值
