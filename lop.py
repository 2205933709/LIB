...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        def extract_key_number(filename):
            match = re.search(r'-(\d+)-[^-]+-(\d+)', filename)  # 匹配任务号
            if match:
        ...
        # 如果只有一行，则不执行后续逻辑
        if len(all_data) == 1:
            print("只有一行数据，跳过分组和聚合。")
            # 在这里处理单行数据，或者返回原始数据
            merged_data = all_data  # 如果需要的话
        else:

            all_data['key_numbers'] = all_data['文件名'].apply(extract_key_number)

            # 过滤掉没有匹配关键数字的行（如果有的话）
            all_data = all_data.dropna(subset=['key_numbers'])
            ...
            # 分组并应用 aggregate_rows 函数，但不包括 'key_numbers' 在 group_dict 中
            merged_data = all_data.groupby('key_numbers').apply(aggregate_rows).reset_index()

            # 在聚合后的数据上重新生成序号
            merged_data['序号'] = range(1, len(merged_data) + 1)

            # 删除其他不再需要的列
            merged_data = merged_data.drop(columns=['key_numbers'])
    ...
    if selected_column:
        if selected_column in scatter_plot_columns:
            ...
            fig.update_xaxes(tickmode='array', tickvals=list(range(len(merged_data))), ticktext=merged_data.index)
            st.plotly_chart(fig)
        else:
            # 对于非散点图列，绘制折线图
